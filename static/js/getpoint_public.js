    ...
    var d = {
        onSearchComplete: function(w) {
            ...
            if (b.getStatus() == BMAP_STATUS_SUCCESS) {
                ...
                if (l == 1 && typeof w.getPoi(0).city == "undefined") {
                    ...
                    if (b && w && w.getNumPois() == 2) {
                        ...
                        if (b && w && w.city) {
                            ...
                            // if (b._json.current_city.name == "å…¨å›½") {
                            //     f = 4
                            // }
                            p = '<span style="color:#00c">å·²åˆ‡æ¢è‡³' + w.city + "</span>";
                    ...
                    a = [];
                    Fe.G("txtPanel").innerHTML = p;
                    Fe.G("resultNum").innerHTML = "å…±æ‰¾åˆ°" + e + "æ¡ç»“æžœ";
                ...
                Fe.G("resultNum").innerHTML = "";
                if (w.city != "å…¨å›½") {
                    y = "åœ¨<b>" + w.city + "</b>åŠå…¨å›½æ²¡æœ‰æ‰¾åˆ°ç›¸å…³çš„åœ°ç‚¹ã€‚"
                } else {
                    y = "åœ¨å…¨å›½æ²¡æœ‰æ‰¾åˆ°ç›¸å…³çš„åœ°ç‚¹ã€‚"
                }
                if (w.getCityList().length > 0) {
                    if (w.city != "å…¨å›½") {
                        y = "åœ¨<b>" + w.city + "</b>æ²¡æœ‰æ‰¾åˆ°ç›¸å…³çš„åœ°ç‚¹ã€‚"
                    } else {
                        y = "åœ¨ä»¥ä¸‹åŸŽå¸‚æœ‰ç»“æžœï¼Œè¯·æ‚¨é€‰æ‹©ï¼š<br />"
                    }
                    var g = w.getCityList();
                    if (g.length > 0) {
                        y += '<p style="margin-top:10px;">åœ¨ä»¥ä¸‹åŸŽå¸‚æ‰¾åˆ°ç»“æžœï¼Œè¯·é€‰æ‹©åŸŽå¸‚ï¼š</p><ul class="SearchList" id="cityList" style="height:40px;overflow:hidden;">';
                        ...
                        y += "</ul>";
                        if (g.length > 6) {
                            y += '<div id="moreCityPop"><a onclick="showMoreCity()" class="resultMore">æ›´å¤šåŸŽå¸‚</a></div>'
                        }
                    }
                }
...
function setCurrentCity(d) {
    if (!d.content || d.content.error == 0) {
        setTimeout(function() {
            ...
            Fe.G("selCityMessage").innerHTML = "è¯·è¾“å…¥æ­£ç¡®çš„ä¸­æ–‡åŸŽå¸‚åç§°"
        },
        0)
    } else {
        hidePop();
        ...
        clearLastResult();
        Fe.G("resultNum").innerHTML = "";
        Fe.G("txtPanel").innerHTML = '<span style="color:#00c">å·²åˆ‡æ¢è‡³' + d.content.cname + "</span>"
    }
}
function hidePop() {
    if (Fe.G("selCityInput")) {
        Fe.G("selCityInput").value = "è¯·è¾“å…¥åŸŽå¸‚å";
...
// åˆ¤æ–­æ˜¯å¦å¸‚ç‰¹æ®ŠåŸŽå¸‚
function isInArray(value){
    var arr = ['é’“é±¼å²›','èµ¤å°¾å±¿','å°åŒ—','é«˜é›„','å°æ¹¾','å°æ¹¾çœ','å°åŒ—å¸‚','é«˜é›„å¸‚'];
...
// å¤„ç†ç‰¹æ®ŠåŸŽå¸‚çš„çŠ¶æ€
function trickCity(cityName) {
    Fe.G("txtPanel").innerHTML = '<span style="color:#00c">å·²åˆ‡æ¢è‡³' + cityName + "</span>";
    switch (cityName) {
        case 'é’“é±¼å²›':
...
function searchByPoint(c) {
    ...
    if (d[0] && d[1]) {
        ...
        function(h) {
            if (h.address) {
                Fe.G("txtPanel").innerHTML = "<b>åœ°å€ï¼š</b>" + h.address
            }
        });
        if (e) {
            e.show();
    ...
    createHtml: function() {
        ...
        if (f < 1) {} else {
            if (this.page >= this.pagecap) {
                b.push('<span><a href="javascript:void(0)" onclick="Instance(\'' + this.hashCode + "').toPage(1);\">é¦–é¡µ</a></span>")
            }
            b.push('<span><a href="javascript:void(0)" onclick="Instance(\'' + this.hashCode + "').toPage(" + f + ');">ä¸Šä¸€é¡µ</a></span>')
        }
        if (this.page < this.pagecap) {
        ...
        if (e > this.pageCount) {} else {
            b.push('<span><a href="javascript:void(0)" onclick="Instance(\'' + this.hashCode + "').toPage(" + e + ');">ä¸‹ä¸€é¡µ</a></span>')
        }
        b.push("</p>");
        return b.join("")
    },
...
Fe.extend(Popup.prototype, {
    render: function() {
        ...
        if (b.isTitle != false) {
            this.title = beforeEndHTML(a, '<div class="title">ç³»ç»Ÿä¿¡æ¯</div>')
        }
